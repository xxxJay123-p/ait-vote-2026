...
const VotingApp = () => {
  ...
  return (
    <>
      ...
      {/* Main Layout */}
      <div className="min-h-screen relative z-10 flex flex-col items-center p-4 md:p-8">
        
        {/* Header Section */}
        <header className="w-full max-w-4xl mb-8 mt-4 text-center relative group">
           {/* Top "Neon Sign" Container */}
           <div className="glass-panel p-8 rounded-xl border border-pink-500/30 relative overflow-hidden">
             {/* Decorative Chinese text top corners */}
             <div className="absolute top-2 left-3 text-xs text-cyan-400 opacity-60 tracking-[0.2em]">香港 • HONG KONG</div>
             <div className="absolute top-2 right-3 text-xs text-pink-400 opacity-60 tracking-[0.2em]">1984</div>

             <div className="relative z-10 flex flex-col items-center">
               <div className="flex items-center gap-3 mb-2 animate-float">
                  <Star className="w-6 h-6 text-yellow-400 fill-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]" />
                  <span className="text-yellow-400 font-bold tracking-widest text-sm drop-shadow-[0_0_5px_rgba(250,204,21,0.5)]">
                    ANNUAL GALA NIGHT
                  </span>
                  <Star className="w-6 h-6 text-yellow-400 fill-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]" />
               </div>
               
               <h1 className="text-5xl md:text-7xl font-black mb-2 hk-title-font neon-text-pink leading-tight">
                 年度晚宴
               </h1>
               <h2 className="text-3xl md:text-5xl font-bold uppercase tracking-tighter neon-text-cyan flex items-center gap-4">
                 <span className="hidden md:inline-block w-12 h-1 bg-cyan-400 shadow-[0_0_10px_#00F0FF]"></span>
                 最佳服裝獎
                 <span className="hidden md:inline-block w-12 h-1 bg-cyan-400 shadow-[0_0_10px_#00F0FF]"></span>
               </h2>
             </div>
             ...
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-pink-600/20 rounded-full blur-[60px] pointer-events-none"></div>
           </div>
        </header>
        ...
        <div className="w-full max-w-4xl flex flex-col md:flex-row gap-4 mb-8">
            <button
              ...
              className={`flex-1 py-4 px-6 rounded-lg font-bold text-xl tracking-wider transition-all duration-300 btn-press border uppercase flex items-center justify-center gap-2 ${
                view === 'home' 
            ...
            >
             ...
             <button
              ...
              className={`py-4 px-6 rounded-lg font-bold transition-all duration-300 btn-press border flex items-center justify-center ${
                showQR 
                  ? 'bg-yellow-400/20 border-yellow-400 text-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.3)]' 
                  : 'bg-black/20 border-white/10 text-gray-400 hover:text-yellow-200 hover:border-yellow-400/50'
              }`}
            >
            ...
            {isAdmin && (
              <button
                ...
                className={`py-4 px-6 rounded-lg font-bold text-xl tracking-wider transition-all duration-300 btn-press border uppercase flex items-center justify-center gap-2 ${
                  view === 'admin' 
                    ? 'bg-black/40 border-orange-500 neon-border-pink text-orange-50' 
                    : 'bg-black/20 border-white/10 text-gray-400 hover:text-orange-200 hover:border-orange-400/50'
                }`}
              >
            ...
            {!isAdmin && (
              <button
                ...
                className="py-4 px-6 rounded-lg font-bold transition-all duration-300 btn-press border bg-black/20 border-white/10 text-gray-400 hover:text-gray-200 hover:border-white/30 flex items-center justify-center"
                title="Admin Login"
              >
        ...
        {showAdminLogin && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setShowAdminLogin(false)}>
            <div className="glass-panel p-8 rounded-2xl max-w-md w-full text-center border-orange-500/50 shadow-[0_0_30px_rgba(249,115,22,0.2)]" onClick={e => e.stopPropagation()}>
              <div className="text-orange-400 text-2xl font-bold mb-4 tracking-widest flex items-center justify-center gap-2">
              ...
              <input
                ...
                className="w-full glass-input px-4 py-3 rounded text-lg placeholder-gray-500 mb-4"
                autoFocus
              />
              <div className="flex gap-3">
                <button
                  onClick={handleAdminLogin}
                  className="flex-1 bg-orange-600 hover:bg-orange-500 text-white px-6 py-3 rounded font-bold uppercase tracking-wider transition-all shadow-[0_0_15px_rgba(249,115,22,0.4)]"
                >
                  Login
                </button>
                <button
                  ...
                  className="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded font-bold uppercase tracking-wider transition"
                >
                  Cancel
                </button>
              </div>
        ...
        {showQR && (
           <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setShowQR(false)}>
             <div className="glass-panel p-8 rounded-2xl max-w-sm w-full text-center border-yellow-500/50 shadow-[0_0_30px_rgba(250,204,21,0.2)]" onClick={e => e.stopPropagation()}>
               <div className="text-yellow-400 text-2xl font-bold mb-4 tracking-widest neon-text-orange">SCAN TO VOTE</div>
               <div className="bg-white p-4 rounded-lg inline-block mb-4 shadow-[0_0_15px_rgba(255,255,255,0.5)]">
                 <img 
               ...
               <button onClick={() => setShowQR(false)} className="mt-6 w-full py-3 bg-yellow-500/20 hover:bg-yellow-500/40 border border-yellow-500/50 text-yellow-300 rounded uppercase tracking-wider font-bold transition">Close</button>
             </div>
           </div>
        )}

        {/* Gender Toggle Switch */}
        <div className="w-full max-w-4xl glass-panel p-2 rounded-xl flex mb-8 relative">
           <div 
             className={`absolute top-2 bottom-2 w-[calc(50%-8px)] bg-gradient-to-r transition-all duration-500 rounded-lg shadow-lg z-0 ${
               gender === 'male' 
               ? 'left-2 from-blue-900 to-blue-600 shadow-[0_0_20px_rgba(37,99,235,0.4)]' 
               : 'left-[calc(50%+4px)] from-pink-900 to-pink-600 shadow-[0_0_20px_rgba(219,39,119,0.4)]'
             }`}
           />
           
           <button
             onClick={() => setGender('male')}
             className={`flex-1 relative z-10 py-3 text-center font-black text-lg md:text-2xl uppercase tracking-widest transition-colors ${
        ...
        {(errorMessage || successMessage) && (
          <div className="w-full max-w-4xl mb-4">
            {errorMessage && (
              <div className="glass-panel p-4 rounded-xl border-l-4 border-l-red-500 bg-red-500/10 animate-fade-in">
            ...
            {successMessage && (
              <div className="glass-panel p-4 rounded-xl border-l-4 border-l-green-500 bg-green-500/10 animate-fade-in">
                <div className="flex items-center gap-3">
        ...
        <div className="w-full max-w-4xl">
          {view === 'home' && (
            <div className="flex flex-col gap-6">
              ...
              <div className="glass-panel p-6 rounded-xl border-l-4 border-l-pink-500">
                <h3 className="text-xl font-bold text-pink-400 mb-4 flex items-center gap-2 uppercase tracking-wider">
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 glass-input px-4 py-4 rounded text-lg placeholder-gray-500"
                  />
                  <button
                    onClick={addCandidate}
                    disabled={!newCandidate.trim()}
                    className="bg-pink-600 hover:bg-pink-500 disabled:opacity-50 disabled:cursor-not-allowed text-white px-8 rounded font-black uppercase tracking-wider transition-all shadow-[0_0_15px_rgba(236,72,153,0.4)] hover:shadow-[0_0_25px_rgba(236,72,153,0.6)]"
                  >
                    Add
                  </button>
                </div>
              ...
              <div className="glass-panel p-6 rounded-xl min-h-[400px]">
                 <h3 className="text-xl font-bold text-cyan-400 mb-6 flex items-center gap-2 uppercase tracking-wider">
                  <Sparkles className="w-5 h-5" />
                  Cast Your Vote
                </h3>

                {hasUserVoted(gender) && (
                  <div className="mb-6 bg-green-500/20 border border-green-500/50 p-4 rounded text-green-300 flex items-center justify-center gap-3 shadow-[0_0_15px_rgba(34,197,94,0.2)]">
                    <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                    Vote Confirmed • 投票已確認
                  </div>
                )}

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {loading ? (
                    <div className="col-span-full py-12 text-center text-gray-500 animate-pulse">Loading neon database...</div>
                  ) : getFilteredCandidates().length === 0 ? (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-gray-700 rounded-xl text-gray-500 flex flex-col items-center">
                      <Users className="w-12 h-12 mb-4 opacity-20" />
                      No candidates yet. Be the first to nominate!
                    </div>
                  ) : (
                    getFilteredCandidates().map((candidate) => (
                      <button
                        ...
                        className={`group relative p-5 rounded-lg border transition-all duration-300 text-left overflow-hidden ${
                          hasUserVoted(gender)
                            ? 'bg-black/40 border-gray-800 opacity-50 cursor-default'
                            : 'bg-black/40 border-gray-700 hover:border-cyan-400 hover:shadow-[0_0_20px_rgba(0,240,255,0.2)]'
                        }`}
                      >
                         ...
                         <div className="flex justify-between items-center relative z-10">
                           <span className={`font-bold text-xl tracking-wide group-hover:text-cyan-300 transition-colors ${hasUserVoted(gender) ? 'text-gray-500' : 'text-gray-200'}`}>
                             {candidate.name}
                           </span>
                           <span className="text-xs text-gray-600 font-mono tracking-widest uppercase">ID:{candidate.id.slice(-4)}</span>
                         </div>
                         
                         {/* Vote Count Badge (visible after voting or strictly transparent?) Let's show it for excitement */}
                         <div className="mt-3 flex items-center justify-between">
                            ...
                            <span className="text-cyan-500 font-mono text-sm">{votes[candidate.id] || 0} PTS</span>
                         </div>
                      </button>
                    ))
                  )}
          ...
          {view === 'leaderboard' && (
            /* LEADERBOARD VIEW */
            <div className="glass-panel p-6 md:p-8 rounded-xl border-t-4 border-t-yellow-500 min-h-[600px]">
               <h3 className="text-3xl font-black text-center mb-10 uppercase tracking-widest neon-text-orange flex justify-center items-center gap-4">
               ...
               <div className="space-y-6">
                 {getFilteredCandidates().length === 0 ? (
                    <div className="text-center text-gray-500 py-10">Waiting for data stream...</div>
                 ) : (
                   getFilteredCandidates().map((candidate, index) => {
                     const voteCount = votes[candidate.id] || 0;
                     const totalVotes = Object.values(votes).reduce((a, b) => a + b, 0); // Total of ALL votes? Or just this gender? The map contains all. We need to filter total by gender ideally, but for visualization relative to max is fine.
                     // Let's approximate max width based on highest vote
                     ...
                     return (
                       <div key={candidate.id} className="relative">
                         {/* Rank Number */}
                         <div className="absolute -left-4 -top-3 w-10 h-10 flex items-center justify-center font-black text-xl italic z-20"
                              style={{ 
                         ...
                         <div className={`
                            relative p-4 pl-8 rounded border-r-4 transition-all duration-500
                         ...
                         `}>
                            <div className="flex justify-between items-end mb-2 relative z-10">
                              <span className={`text-xl font-bold tracking-wider ${index === 0 ? 'text-yellow-100' : 'text-gray-300'}`}>
                                {candidate.name}
                              </span>
                              <span className="font-mono text-2xl font-bold neon-text-pink">
                                {voteCount}
                              </span>
                            </div>
               ...
               {/* Footer Stats */}
               <div className="mt-12 pt-6 border-t border-gray-800 flex justify-between text-xs font-mono text-gray-500 uppercase tracking-widest">
                 <span>System: Online</span>
                 <span>Loc: Wan Chai / HK</span>
               </div>
            </div>
          )}
          {view === 'admin' && (
            /* ADMIN PANEL */
            <div className="glass-panel p-6 md:p-8 rounded-xl border-t-4 border-t-orange-500 min-h-[600px]">
              <h3 className="text-3xl font-black text-center mb-10 uppercase tracking-widest text-orange-400 flex justify-center items-center gap-4">
              ...
              <div className="space-y-6">
                ...
                <div className="glass-panel p-6 rounded-xl border-l-4 border-l-cyan-500">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`font-bold ${isVotingAllowed() ? 'text-green-400' : 'text-red-400'}`}>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full glass-input px-4 py-2 rounded"
                      />
                    </div>
                    <div>
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex-1 bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded font-bold uppercase tracking-wider transition"
                      >
                        Enable Voting
                      </button>
                      <button
                        onClick={() => updateVotingSettings({ isActive: false })}
                        className="flex-1 bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded font-bold uppercase tracking-wider transition"
                      >
                        Disable Voting
                      </button>
                    </div>
                    <button
                      ...
                      className="w-full bg-gray-600 hover:bg-gray-500 text-white px-4 py-2 rounded font-bold uppercase tracking-wider transition"
                    >
                      Clear Time Limits
                    </button>
                  </div>
                ...
                <div className="glass-panel p-6 rounded-xl border-l-4 border-l-red-500">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full bg-orange-600 hover:bg-orange-500 text-white px-4 py-3 rounded font-bold uppercase tracking-wider transition shadow-[0_0_15px_rgba(249,115,22,0.4)]"
                    >
                      Reset All Votes
                    </button>
                    <button
                      onClick={clearAllData}
                      className="w-full bg-red-600 hover:bg-red-500 text-white px-4 py-3 rounded font-bold uppercase tracking-wider transition shadow-[0_0_15px_rgba(220,38,38,0.4)]"
                    >
                ...
                <div className="flex justify-center pt-4">
                  <button
                    ...
                    className="bg-gray-700 hover:bg-gray-600 text-white px-6 py-2 rounded font-bold uppercase tracking-wider transition"
                  >
                    Logout Admin
                  </button>
                </div>
